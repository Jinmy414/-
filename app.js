...
const TYPE_OPTIONS = ['原创', '轻改', '漫改', '游戏改', '特摄', '机战', '异世界', '热血', '奇幻', '玄幻', '科幻', '校园', '搞笑', '日常', '恋爱', '百合', '运动', '智斗', '偶像', '职场'];
...
function loadState() {
  try {
    ...
    return {
      ...
      works: Array.isArray(saved.works) && saved.works.length ? saved.works : structuredClone(SEED_WORKS)
    };
...
function renderHome() {
  ...
  return `<section class="hero">
    <div class="hero-copy"><div class="eyebrow">A gentle place for good stories</div><h1>把喜欢的作品，<br /><em>慢慢放在一起。</em></h1><p class="hero-intro">罗瓣是一个轻一点的作品社区。来这里发现新故事，留下半分认真或一句随口的喜欢，也看看别人为什么记住了它。</p><div class="hero-actions"><button class="button button-primary" type="button" data-scroll="discover">开始发现 <span>↗</span></button><button class="button button-ghost" type="button" data-open-create>创建一部作品</button></div><div class="hero-stats"><div class="stat"><strong>${state.works.length}</strong><span>正在被记录的作品</span></div><div class="stat"><strong>${state.works.reduce((sum, work) => sum + work.comments.length, 0)}</strong><span>留下的片段</span></div><div class="stat"><strong>0.5</strong><span>评分最小刻度</span></div></div></div>
    <div class="hero-art"><span class="art-label">LUOBAN / 01</span><div class="art-logo-frame"><img src="assets/logo-snail.png" alt="罗瓣 logo" /></div><span class="art-caption">见喜欢，见自己</span></div>
  </section>
  <section id="discover"><div class="section-heading"><div><h2>今日推荐</h2><p>每次打开，遇见 3 部热门高分和 3 部冷门高分作品。</p></div><span class="text-link">随机漫游中 · ${new Date().toLocaleDateString('zh-CN', { month: 'long', day: 'numeric' })}</span></div>
  ...
  <section class="works-section"><div class="section-heading"><div><h2>全部作品</h2><p>用标签收拢兴趣，按自己的节奏筛选。</p></div></div>${renderFilters()}<div class="works-grid">${filtered.length ? filtered.map(work => workCard(work)).join('') : '<div class="empty-state"><strong>没有找到匹配的作品</strong>换个关键词或放宽筛选条件试试。<br /><button class="button button-ghost button-small" type="button" data-clear-filters>清除筛选</button></div>'}</div></section>`;
}
